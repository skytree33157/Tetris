...
- `addDropScore(int distance, DropType type)`
...
```
scoreManager.addDropScore(1, DropType.AUTO);
scoreManager.addDropScore(1, DropType.SOFT);
scoreManager.addDropScore(8, DropType.HARD);
```
...
⚠️⚠️줄을 삭제하지 않은 경우에도 `linesCleared = 0`으로 호출해야 Combo가 초기화됩니다!!⚠️⚠️

- `getScore()`
    - 현재 누적 점수를 반환합니다. UI에서는 이 값을 이용해 현재 점수를 표시할 수 있습니다.
```
...
Level, Combo, Perfect Clear 배율을 먼저 곱해, 먼저 `rawMultiplier`를 계산합니다.
...
현재 최대 `rawMultiplier`는 20으로 설정되어 있습니다.
